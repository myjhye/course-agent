...
class EnrollmentService:
    ...
    async def create_enrollment(db: AsyncSession, data: EnrollmentCreate) -> Enrollment:
        ...
        중복을 허용하면 수강 이력이 꼬이고 추천·통계 데이터가 오염된다.
        """
    ...
    async def get_enrollments_by_student(db: AsyncSession, student_name: str) -> List[dict]:
        ...
        result = await db.execute(
            ...
            .order_by(desc(Enrollment.created_at))  # 최신 수강이 먼저 보이도록
        )
    ...
    async def get_all_enrollments_paginated(
        ...
        # 조건이 없으면 전체 조회, 있으면 and_로 묶어 필터링
        conditions = []
        if status:
        ...
        # 페이지네이션 메타(total_pages) 계산을 위해 필터 적용 후 전체 수를 먼저 센다
        ...
        return {
            ...
            "page": page,
            "page_size": page_size,
            "total_pages": (total + page_size - 1) // page_size  # 나머지 있으면 올림해 마지막 페이지 포함
        }
    ...
    async def get_enrollment_by_id(db: AsyncSession, enrollment_id: int) -> Optional[Enrollment]:
        """
        수강 ID로 단건을 조회한다.
        update_enrollment·cancel_enrollment·피드백 생성에서 대상이 존재하는지 확인하는 용도로 쓴다.
        없으면 None을 반환한다.
        """
        result = await db.execute(
            select(Enrollment)
