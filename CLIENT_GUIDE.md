...
* **`ChatPage.tsx`**
  * **역할**: **핵심 AI 스포츠 비서와 대화하는 채팅 뷰**입니다. 사용자의 질문을 백엔드 스트리밍 엔드포인트로 요청하고, 백엔드로부터 전송받은 실시간 에이전트 상태(Supervisor ➡️ Lesson ➡️ FAQ 등)를 시각적인 상태 로더로 연동하여 단계별 흐름을 사용자에게 연출합니다.
