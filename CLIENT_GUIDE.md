...
* **`ChatPage.tsx`**
  * **역할**: **AI 스포츠 비서와의 채팅 뷰**입니다. 사용자의 질문을 백엔드 스트리밍 엔드포인트로 전송하여 SSE 상태와 답변을 출력합니다.
  * **연계 개선 사항**:
    1. **마크다운 이미지 최적화**: `<ReactMarkdown>` img 렌더러 커스텀(`components={{ img: ... }}`)을 통해 RAG 연계 이미지들의 크기를 모바일 280px / PC 360px로 제한하고 둥근 모서리 및 호버 효과를 적용해 가독성을 높였습니다.
    2. **새 대화 유도 웰컴 카드**: 세션 이력이 0건이 되어 비었을 때(New Chat 등) 휑한 공백 대신 "대화 내역이 없습니다. 새 대화를 시작해 보세요!" 라는 안내 가이드 문구와 아이콘을 띄우는 UX를 구축했습니다.
